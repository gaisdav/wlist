...
- [ ] Миграции: создание таблиц `events`, `event_wishes`
- [ ] Настройка каскадного удаления (`on delete cascade`) для связей
- [ ] Настройка политик RLS для `events` and `event_wishes`
- [ ] API + hooks для работы с событиями (получение, создание, редактирование, удаление)
- [ ] UI: Создать универсальный компонент `Badge` (с поддержкой разных размеров и цветов) в `apps/tma/src/components/primitives`
- [ ] UI: секция «События» на профиле пользователя
- [ ] UI: создание/редактирование события
- [ ] UI: экран детального просмотра события (список желаний)
- [ ] UI: добавление мультиселекта событий в форму создания/редактирования желания
- [ ] UI: Использовать `Badge` для вывода привязанных к желанию событий в `WishCard.tsx` и `WishDetailPage.tsx`
