...
**Emoji first.** This app uses emoji as the primary icon system — it's a core design feature, not a fallback.

- Default: use an emoji (e.g. `💸`, `📂`, `✅`, `⚙️`)
- Custom icon component / SVG / icon library: only in rare, justified exceptions where emoji cannot convey the meaning or the context demands pixel-precise control (e.g. animated loading indicator, platform-native tab bar icon)
- During refactoring: if you find icon library usage, replace with emoji unless there is a clear reason not to
