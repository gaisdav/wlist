...
import { SUPPORTED_WISH_CURRENCIES } from '@wlist/core/lib';
...
export const WishFormPage = ({ mode }: WishFormPageProps): React.JSX.Element => {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      priceStr: existing.data.price != null ? String(existing.data.price) : '',
      currency: existing.data.currency,
      linkStr: existing.data.link ?? '',
    });
  }, [existing.data, reset]);
    ...
    <form className="flex flex-col gap-4 p-4" onSubmit={handleSubmit(onValid)}>
      ...
      <label className="flex flex-col gap-1">
        ...
        {formState.errors.currency ? (
          <span className="text-xs text-destructive">{formState.errors.currency.message}</span>
        ) : null}
      </label>
